...
/** Earliest series year offered in ordinance/resolution upload forms. */
export const SERIES_YEAR_START = 1980;

/**
 * Series years from {@link SERIES_YEAR_START} through the current calendar year
 * (newest first). Extends automatically each year (e.g. 2027, 2028).
 */
export function getSeriesYearOptions(
  throughYear: number = new Date().getFullYear()
): number[] {
  const end = Math.max(throughYear, SERIES_YEAR_START);
  const years: number[] = [];
  for (let year = end; year >= SERIES_YEAR_START; year--) {
    years.push(year);
  }
  return years;
}
