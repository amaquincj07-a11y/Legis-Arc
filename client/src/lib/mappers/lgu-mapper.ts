...
export interface LguRow {
  ...
  province: string;
  municipality: string;
  /** DB may return `pending` / `paid`; mapped to client statuses in `mapLguRowToClient`. */
...
export function mapLguRowToClient(row: LguRow): LGUClient {
  ...
  const administrator: LGUAdministrator = {
    ...
    managedPassword:
      typeof primaryAdmin?.managed_password === "string" &&
      primaryAdmin.managed_password.length > 0
        ? primaryAdmin.managed_password
        : undefined,
  };

  // Postgres enum uses `pending` for unpaid trial accounts.
  const status: LGUClientStatus =
    row.status === "pending" || row.status === "trial"
      ? "trial"
      : row.status === "paid"
        ? "active"
        : row.status;
